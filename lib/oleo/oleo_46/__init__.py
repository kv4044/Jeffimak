...
def janela_oleo_46(janela_principal):
    janela_principal.withdraw()

    oleo_46 = tk.Toplevel()
    oleo_46.geometry('1280x720')
    oleo_46.configure(bg='grey')
    oleo_46.title('CONTROLE DE ÓLEO')

    titulo = tk.Label(oleo_46, text="ÓLEO 46", bg="#5e34eb", font=('helvica', 16, 'bold'))
    titulo.pack(side=tk.TOP, fill=tk.X, ipady=20)

    tk.Label(oleo_46, text='Equipamento:', font=('helvica', 14, 'bold'), bg='grey').place(x=100, y=140)
    entrada_e = tk.Entry(oleo_46, font=('helvica', 14, 'bold'))
    entrada_e.place(x=100, y=170)

    tk.Label(oleo_46, text='Quantidade retirada:', font=('helvica', 14, 'bold'), bg='grey').place(x=100, y=230)
    global entrada_retirada
    entrada_retirada = tk.Entry(oleo_46, font=('helvica', 14, 'bold'))
    entrada_retirada.place(x=100, y=260)

    global label_nao_usado
    global label_usado
    label_nao_usado = tk.Label(oleo_46, text="", font=('helvica', 14, 'bold'), bg='grey')
    label_nao_usado.place(x=330, y=170)
    label_usado = tk.Label(oleo_46, text="", font=('helvica', 14, 'bold'), bg='grey')
    label_usado.place(x=330, y=260)

    # Inicializa o óleo selecionado e atualiza os labels
    global oleo_selecionado
    oleo_selecionado = "Oleo 46"
    oleo = Oleo.get(Oleo.nome == oleo_selecionado)
    atualizar_labels(oleo)

    tk.Button(oleo_46, text='Atualizar', bg='#81ff1a', command=processar_entrada,
              font=('helvica', 12, 'bold')).place(x=100, y=300)

    tk.Button(oleo_46, text='Reiniciar', bg='#ff1a1a', command=reiniciar_oleo_click,
              font=('helvica', 12, 'bold')).place(x=200, y=300)

    def alternar_tela_cheia(event=None):
        estado_atual = oleo_46.attributes('-fullscreen')
        oleo_46.attributes('-fullscreen', not estado_atual)

    oleo_46.attributes('-fullscreen', True)

    botao_sair = tk.Button(oleo_46, text='Sair', bg='#81ff1a',
                           command=lambda: fechar_janelacadastro(oleo_46, janela_principal),
